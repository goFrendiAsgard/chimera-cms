...
function cwAdjustDflexTables () {
  $('tr.d-flex').each(function () {
    ...
    if (totalWidth > allowedWidth) {
      console.log([totalWidth, allowedWidth])
  ...
  $('table').each(function () {
    ...
    if (thead.children('tr').length > 0 && tbody.children('tr').length > 0) {
      let trHead = thead.children('tr')
      let trBody = tbody.children('tr')
      trHead.width(trBody.width())
    }
  })
}
